...
module ipu_flow_control(
    ...
    logic [6*YUV422_WIDTH-1:0] yuv422_pipe;
    logic [6*RGB888_WIDTH-1:0] rgb888_pipe;
    logic [6*RGB565_WIDTH-1:0] rgb565_pipe;
    logic [6*RAW8_WIDTH-1:0]  raw8_pipe;
    logic [6*RAW10_WIDTH-1:0] raw10_pipe;
    logic fifo_rvalid;
    ...
    assign pipe_insert = (ram_valid & sync_state==FRAME_VALID & pixel_data_valid!=4'h0);
    always_ff@(posedge pixel_clk_i or negedge pixel_reset_n_i) begin
        if(!pixel_reset_n_i) begin
            ...
            if(line_done_pulse)
                ...
                FRAME_VALID: begin
                    ...
                    else if(ram_valid && (pixel_data_valid==4'h0) && line_done_latch)
                        sync_state <= FRAME_INVALID;
                end
                LINE_VALID: begin
                    line_valid_sync_r <= 1;
    ...
    assign line_done_pulse = !line_done_latch & ram_valid & (pixel_data_valid==4'h0) & sync_state==FRAME_VALID;
            ...
            if(pipe_insert) begin
                for(int i=0; i<4; i++) begin
                    if(pixel_data_valid[i]) begin
                        yuv422_pipe [(i+empty_space)*YUV422_WIDTH  +: YUV422_WIDTH]    <= pixel_data[i*YUV422_WIDTH +: YUV422_WIDTH];
                        rgb888_pipe [(i+empty_space)*RGB888_WIDTH  +: RGB888_WIDTH]    <= pixel_data[i*RGB888_WIDTH +: RGB888_WIDTH];
                        rgb565_pipe [(i+empty_space)*RGB565_WIDTH  +: RGB565_WIDTH]    <= pixel_data[i*RGB565_WIDTH +: RGB565_WIDTH];
                        raw8_pipe   [(i+empty_space)*RAW8_WIDTH    +: RAW8_WIDTH]      <= pixel_data[i*RAW8_WIDTH   +: RAW8_WIDTH];
                        raw10_pipe  [(i+empty_space)*RAW10_WIDTH   +: RAW10_WIDTH]     <= pixel_data[i*RAW10_WIDTH  +: RAW10_WIDTH];
                    end
                end
            end
    ...
    `ifndef ASIC
        `ifndef FPGA
            ...
            property ineqout_val;
                @(posedge pixel_clk_i)
                //(posedge_line_valid) |-> ##1 ($past(ycounter) == 'd960);
                (pixel_data_valid_o) |-> (pixel_data_o[19:0] == $past(byte_data_i[19:0], 10)) || (pixel_data_o[19:0] == $past(byte_data_i[19:0], 9)) || (pixel_data_o[19:0] == $past(byte_data_i[19:0], 8)) || (pixel_data_o[19:0] == $past(byte_data_i[39:20], 10)) || (pixel_data_o[19:0] == $past(byte_data_i[39:20], 9)) || (pixel_data_o[19:0] == $past(byte_data_i[39:20], 8));
            endproperty

            assert property (ineqout_val) else $fatal("Assertion failed (flow control): output doesn't eq input, output =%h, input=%h",pixel_data_o[19:0], $past(byte_data_i[19:0], 9));
